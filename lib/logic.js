...
async function payOut(shipmentReceived) {  // eslint-disable-line no-unused-vars
    ...
    if (shipmentReceived.timestamp > contract.arrivalDateTime) {
        ...
        if (shipment.temperatureReadings) {
          ...
          console.log('contract min temp reading: ' + contract.minTemperature);
          ...
          let totalPenalty = penalty * shipment.unitCount
    ...
    console.log('Payout: ' + payOut);
 	contract.importer.accountBalance -= payOut;
  	let shipperCommisionAmount = payOut * ( contract.shipperCommisionInPercent / 100 ) ;
	console.log('shipperCommisionAmount: ' + shipperCommisionAmount);
  	contract.shipper.accountBalance += shipperCommisionAmount;
    contract.grower.accountBalance  += payOut - shipperCommisionAmount;
    ...
    await shipmentRegistry.update(shipment);
   	...
   	const shipperRegistry = await getParticipantRegistry('org.acme.shipping.perishable.Shipper');
